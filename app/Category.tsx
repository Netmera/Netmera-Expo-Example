...
const Category = () => {
  ...
  const updateStatusCategories = async () => {
    ...
    Netmera.updateStatusByCategories(0, count, categoryState)
      .then(() => {
  ...
  const setUserCategoryPreference = (item: NMCategoryPreference) => {
    item.categoryId &&
      Netmera.setUserCategoryPreference(item.categoryId, !item.optInStatus)
        .then(() => {
  ...
  const getCategoryItem = (item: NetmeraCategory, index: number) => {
    ...
    return (
      <View style={{paddingHorizontal: 10, width: '100%'}}>
        ...
        <Text>
          Category Status:{' '}
          {item.categoryStatus === undefined ? 'null' : item.categoryStatus}
        </Text>
        <Text>
          Read Count: {item.readCount === undefined ? 'null' : item.readCount}
        </Text>
        <Text>
          Unread Count:{' '}
          {item.unreadCount === undefined ? 'null' : item.unreadCount}
        </Text>
        <Text>
          Deleted Count:{' '}
          {item.deletedCount === undefined ? 'null' : item.deletedCount}
        </Text>
        <Text>
          Last Message:{' '}
  ...
  const getUserCategoryPreferenceItem = (
    item: NMCategoryPreference,
    index: number,
  ) => {
    return (
      <View
        ...
        <View style={{width: '90%', flexDirection: 'row'}}>
          <View style={{width: '70%'}}>
            <Text>
              Category Id:{' '}
              {item.categoryId === undefined ? 'null' : item.categoryId}
            </Text>
            <Text>
              Category Name:{' '}
              {item.categoryName === undefined ? 'null' : item.categoryName}
          ...
          <View style={{width: '30%'}}>
            <TouchableHighlight
              ...
              <Text style={styles.buttonText}>
                {item.optInStatus ? 'Disable' : 'Enable'}
              </Text>
            </TouchableHighlight>
          </View>
        </View>
